...
{
  public readonly ImageProcessRepository imageProcessRepository =
    imageProcessData ?? throw new ArgumentNullException(nameof(imageProcessData));
  ...
  public async Task CreateAppraisalOverlay(List<CharaDataClass> items)
  {
    ...
    {
      if (imageProcessRepository.AppraisalCharaData.Contains(item))
        continue;
