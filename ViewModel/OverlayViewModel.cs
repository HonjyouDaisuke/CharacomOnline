...
public class OverlayViewModel
{
	...
	public void ChangeOverlayColor(bool isBoxA, string color)
	{
		...
		if (target.OverlayBmp == null) return;
	...
	public async Task MakeViewBitmap(bool isCenterLine, bool isGridLine, bool isStandard)
	{
		await Task.Run(() =>
		{
			if (OverlayA == null && OverlayB == null) return;
			...
			else
			{
				...
				{
					ViewBitmap = ImageEffectService.OverlayBinaryImages(OverlayA.OverlayBmp, OverlayB.OverlayBmp);
				}
				else
				{
					ViewBitmap = ImageEffectService.OverlayBinaryImages(OverlayB?.OverlayBmp, OverlayA?.OverlayBmp);
				}
			}
