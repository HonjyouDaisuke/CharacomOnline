...
)
{
  ...
  public async Task<Guid?> InsertCharaDataAsync(string fileName, Guid projectId, string fileId, Guid userId)
  {
    ...
    var charaDataId = await _charaDataTableService.CreateCharaData(
      projectId,
      fileId,
  ...
  {
    Console.WriteLine($"chara = {currentChara} material = {currentMaterial}");
  ...
  public async Task GetStrokeData(string accessToken, string charaName)
  {
    ...
    Console.WriteLine($"ファイルID:{fileId}");
    if (fileId == null)
    {
