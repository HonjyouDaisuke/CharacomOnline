...
{
  private static readonly Dictionary<ProfileMenuItemType, string> _menuItemTypeNames = new()
    {
  ...
  // 文字列からEnumを取得する逆辞書
  private static readonly Dictionary<string, ProfileMenuItemType> _stringToEnumMap = _menuItemTypeNames
      .ToDictionary(kv => kv.Value, kv => kv.Key);

  // 文字列からEnumを取得するメソッド
  public static ProfileMenuItemType GetEnum(string value)
  {
    return _stringToEnumMap.TryGetValue(value, out var itemType) ? itemType : throw new ArgumentException("無効な文字列です", nameof(value));
  }
}