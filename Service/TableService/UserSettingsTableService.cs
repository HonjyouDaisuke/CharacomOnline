...
public class UserSettingsTableService(Client supabaseClient)
{
  ...
  private async Task UpdateSetting<T>(
    Guid userId,
    Expression<Func<UserSettings, T>> selector,
    T value
  )
  {
    try
      ...
      var response = await _supabaseClient
        ...
        .Set(
          selector.Body is MemberExpression
            ? Expression.Lambda<Func<UserSettings, object>>(
  ...
  public Task UpdateIsStandard(Guid userId, bool isSelect) =>
    UpdateSetting(userId, x => x.IsStandard, isSelect);

  public Task UpdateIsCenterLine(Guid userId, bool isSelect) =>
    UpdateSetting(userId, x => x.IsCenterLine, isSelect);

  public Task UpdateIsGridLine(Guid userId, bool isSelect) =>
    UpdateSetting(userId, x => x.IsGridLine, isSelect);

  public Task UpdateIsNoize(Guid userId, bool isSelect) =>
    UpdateSetting(userId, x => x.IsNoize, isSelect);
